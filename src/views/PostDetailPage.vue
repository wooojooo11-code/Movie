...
import { COMMUNITY_CATEGORY_LABELS, type CommunityComment, type CommunityPostDetail } from '@/types/community';
...
const beginEdit = () => {
  ...
  editTitle.value = post.value.title; editContent.value = post.value.content; editImageUrl.value = post.value.imageUrl ?? ''; editHasSpoiler.value = post.value.hasSpoiler; editing.value = true;
};
const saveEdit = async () => {
  if (!post.value || !editTitle.value.trim()) return;
  submitting.value = true;
  try {
    await updateCommunityPost(post.value.id, { title: editTitle.value, content: editContent.value, imageUrl: editImageUrl.value, hasSpoiler: editHasSpoiler.value, movie: post.value.movie });
    editing.value = false; await load();
  } catch { errorMessage.value = '게시글 수정에 실패했습니다.'; }
  finally { submitting.value = false; }
};
const removePost = async () => {
  if (!post.value || !window.confirm('이 게시글을 삭제할까요?')) return;
  try { await deleteCommunityPost(post.value.id); await router.replace('/community'); }
  catch { errorMessage.value = '게시글 삭제에 실패했습니다.'; }
};
const toggleLike = async () => {
  if (!post.value || !viewerId.value) { goToLogin(); return; }
  const before = post.value.viewer.hasLiked; post.value.viewer.hasLiked = !before; post.value.likeCount += before ? -1 : 1;
  try { await toggleCommunityLike(post.value.id, viewerId.value, before); }
  catch { post.value.viewer.hasLiked = before; post.value.likeCount += before ? 1 : -1; errorMessage.value = '좋아요를 반영하지 못했습니다.'; }
};
const toggleSave = async () => {
  if (!post.value || !viewerId.value) { goToLogin(); return; }
  const before = post.value.viewer.hasSaved; post.value.viewer.hasSaved = !before; post.value.saveCount += before ? -1 : 1;
  try { await toggleCommunitySave(post.value.id, viewerId.value, before); }
  catch { post.value.viewer.hasSaved = before; post.value.saveCount += before ? 1 : -1; errorMessage.value = '저장을 반영하지 못했습니다.'; }
};
const toggleFollow = async () => {
  if (!post.value || !viewerId.value) { goToLogin(); return; }
  const before = post.value.viewer.isFollowingAuthor; post.value.viewer.isFollowingAuthor = !before;
  try { await toggleCommunityFollow(post.value.userId, viewerId.value, before); }
  catch { post.value.viewer.isFollowingAuthor = before; errorMessage.value = '팔로우를 반영하지 못했습니다.'; }
};
const report = async () => {
  if (!post.value || !viewerId.value) { goToLogin(); return; }
  if (!window.confirm('이 게시글을 신고할까요?')) return;
  try { await reportCommunityPost(post.value.id, viewerId.value, 'inappropriate'); window.alert('신고가 접수되었습니다.'); }
  catch (error) { errorMessage.value = error instanceof Error ? error.message : '신고에 실패했습니다.'; }
};
const addComment = async (content: string) => {
  ...
  try {
    ...
    comments.value.push(await createComment(post.value.id, viewerId.value, content)); post.value.commentCount += 1;
  } catch { errorMessage.value = '댓글 등록에 실패했습니다.'; }
  finally { submitting.value = false; }
};
const removeComment = async (commentId: string) => {
  if (!post.value) return;
  try { await deleteComment(commentId); comments.value = comments.value.filter((comment) => comment.id !== commentId); post.value.commentCount = Math.max(0, post.value.commentCount - 1); }
  catch { errorMessage.value = '댓글 삭제에 실패했습니다.'; }
};
...
const vote = async (optionId: string) => {
  ...
  try { await castPollVote(post.value.poll.id, optionId, viewerId.value); post.value.poll.viewerOptionId = optionId; await refreshPoll(); }
