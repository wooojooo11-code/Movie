-- 이미 커뮤니티 마이그레이션을 적용한 환경에서도 투표 항목 수 제한을 해제합니다.
alter table public.community_poll_options
  drop constraint if exists community_poll_options_position_check;

alter table public.community_poll_options
  alter column position type integer;

alter table public.community_poll_options
  add constraint community_poll_options_position_check check (position >= 1);
